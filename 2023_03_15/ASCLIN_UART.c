...
uint8 uart_rx_buff_old[7] = { 0, };
uint8 uart_tx_buff[7] = { 0, };
Ifx_SizeT p_count = sizeof(protocal_test);
Ifx_SizeT r_count = sizeof(uart_rx_buff);
Ifx_SizeT o_count = sizeof(uart_rx_buff_old);
Ifx_SizeT t_count = sizeof(uart_tx_buff);
extern int flag = 0;
extern uint8 data_result1 = 0;
extern uint8 data_result2 = 0;
...
int parsing_rx_buff(void)
{
    ...
    if((uart_rx_buff[0] == 0xb8) && (uart_rx_buff[1] == 0xae) )
    {
        if(uart_rx_buff[2] == 'O')
        {
            crc_16_val_P.check_sum = CRC16_MODBUS(uart_rx_buff, 5);

            if((  crc_16_val_P.b[0] == uart_rx_buff[5]) && (  crc_16_val_P.b[1] == uart_rx_buff[6]))
            {
                data_result1 = uart_rx_buff[3];
                data_result2 = uart_rx_buff[4];
            }
        }
    }
    return data_result2;
...
void send_buff(void)
{
    protocal_test[0] = uart_tx_buff[1];                 //PC
    protocal_test[1] = uart_tx_buff[0];                 //AURIX
    protocal_test[2] = 'i';                             //IO
    protocal_test[3] = uart_tx_buff[3];
    protocal_test[4] = uart_tx_buff[4];

    crc_16_val_S.check_sum = CRC16_MODBUS(protocal_test, 5);

    protocal_test[5] = crc_16_val_S.b[0];
    protocal_test[6] = crc_16_val_S.b[1];
}
